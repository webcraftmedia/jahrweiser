...
| `server/assets/map/boundaries.json` | ~18.000 Bundesland and Kreis border arcs at two resolutions, plus 415 names |
...
is stored **twice**: as it is, and thinned to `COARSE_TOLERANCE` (4 ≈ 212 m),
which is nine times fewer vertices. The artefact goes from 2.277 kB to 3.153 kB;
...
at — and above `COARSE_ABOVE` the coarse copy's error is under a pixel and that
is what it gets. A rule based on the requested box would have had to guess at a
screen size and would be wrong on both a phone and a wall display. The resolution
is remembered with the region, so zooming in past it fetches again.
...
The trigger is the width of the view **relative to the whole country** — a scale
rather than a zoom step, so it means the same thing on a phone and on a wall
screen. Germany is about 640 km across.
...
and one at 60 km across is 2 kB. The fetched region carries a smaller margin
than the place names do (0.3 against 0.6) — a name is a few bytes, a Kreis
border a few hundred.
