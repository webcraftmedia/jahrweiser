...
import { buildMapPayload, loadPlzAreas } from '../../helpers/memberMap'
 ...
 * and members who have not given their own postal code get a 403 with no data
 * at all — the blurred preview the page shows them is drawn from made-up
 * numbers on the client. Sending the real thing and blurring it in CSS would
 * hand it to anyone who opens the network tab.
 */
export default defineEventHandler(async (event): Promise<MapPayload> => {
  ...
  if (!geometry) {
    ...
    // cannot draw anything, and an operator should see why in the log.
